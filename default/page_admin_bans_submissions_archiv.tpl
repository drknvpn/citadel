{if NOT $permissions_submissions}
	Доступ закрыт!
{else}
	<h3 style="margin-top:0px;">Архив жалоб (<span id="subcountarchiv">{$submission_count_archiv}</span>)</h3>
	Щелкните на ник игрока, чтобы просмотреть информацию об жалобах на них<br /><br />
    <div id="banlist-nav">
        {$asubmission_nav}
    </div>
		...
		{foreach from="$submission_list_archiv" item="sub"}
			<tr id="asid_{$sub.subid}" class="opener4 tbl_out" {if $sub.hostname == ""}onclick="xajax_ServerHostPlayers('{$sub.server}', 'id', 'suba{$sub.subid}');"{/if} onmouseout="this.className='tbl_out'" onmouseover="this.className='tbl_hover'">
	            ...
	            <td style="border-bottom: solid 1px #ccc" height='16'>
		            ...
		            <a href="#" onclick="xajax_SetupBan({$sub.subid});">Бан</a> -
						...
						<table width="90%" cellspacing="0" cellpadding="0" class="listtable">
							...
							<tr align="left">
									<td height="16" align="left" class="listtable_1" colspan="2">
										<b>Архивировано, потому что {$sub.archive}</b>
									</td>
							...
							<tr align="left">
            					...
            					<td height="" class="listtable_1" id="suba{$sub.subid}">{if $sub.hostname == ""}<i>Получение имени хоста</i>{else}{$sub.hostname}{/if}</td>
      						</tr>
      						<tr align="left">
