...
function set_counter(count)
{
	...
	if(srvcount==-{$total}- || count=='-1') {
		parent.document.getElementById('dialog-control').innerHTML = "<font color=\"green\" style=\"font-size: 12px;\"><b>Поиск завершен.</b></font>"+parent.document.getElementById('dialog-control').innerHTML;
...
<h3 style="font-size: 12px;">Поиск игрока на всех серверах...</h3>
<table border="0">
-{foreach from=$servers item=serv}-
<tr>
	<td><div id="srvip_-{$serv.num}-"><font size="1">-{$serv.ip}-:-{$serv.port}-</font></div></td>
	<td>
		<div id="srv_-{$serv.num}-"><font size="1">Ожидание...</font></div>
	</td>
</tr>
-{/foreach}-
</table>
